...
3. No
- **Calls**: src.fixop.ssh.run_remote, src.fixop.ssh.run_remote, src.fixop.ssh.run_remote, issues.append, logs.stdout.strip, logs.stdout.lower, check.stdout.lower, dns_check.stdout.lower
...
- **Calls**: None.join, src.fixop.ssh.run_remote, Issue, FixResult, result.stdout.strip, result.stderr.strip, enumerate, None.join
...
- **Calls**: src.fixop.ssh.run_remote, src.fixop.ssh.run_remote, issues.append, issues.append, Issue, Issue

### src.fixop.firewall.fix_ufw_allow_routed
> Set UFW DEFAULT_FORWARD_POLICY to ACCEPT and reload.
- **Calls**: src.fixop.ssh.run_remote, Issue, FixResult, result.stdout.strip, result.stderr.strip
...
- **Calls**: src.fixop.ssh.run_remote, issues.append, result.stdout.strip, Issue
...
### src.fixop.ssh.RemoteResult
> Result of a remote command execution.
- **Methods**: 2
- **Key Methods**: src.fixop.ssh.RemoteResult.success, src.fixop.ssh.RemoteResult.output
...
### src.fixop.cli._cmd_validate
> Validate deploy artifacts locally.
- **Output to**: src.fixop.deploy.scan_deploy_dir, src.fixop.cli._output_issues

## Public API Surface

Functions exposed as public API (no underscore prefix):

- `src.fixop.cli.main` - 43 calls
- `src.fixop.tls.check_certificate` - 34 calls
- `src.fixop.check_all` - 22 calls
- `src.fixop.deploy.check_unresolved_vars` - 21 calls
...
- `src.fixop.ssh.test_ssh_connection` - 7 calls
...
- `src.fixop.ssh.run_remote` - 4 calls
- `src.fixop.ports.is_port_free` - 3 calls
- `src.fixop.containers.check_runtime` - 3 calls
